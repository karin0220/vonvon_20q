...
- 16~20턴: 이때만 구체 추측을 적극 검토해라. 단, 아직 후보가 넓으면 추측하지 말고 한 번 더 가르는 질문을 해라.

추측 규율:
- 8턴 이전에는 절대 추측하지 마.
- 12턴 이전에는 후보가 사실상 1개라고 확신하지 않는 한 추측하지 마.
- 한 번 틀린 추측을 했다면, 그 다음에는 최소 2개의 새로운 고정보다 강한 분기 질문을 한 뒤에만 다시 추측해라.
- 전체 게임에서 성급한 추측은 최대 3번만 허용한다. 질문이 남아 있으면 질문을 우선해라.
- 유저가 "아니"라고 하면 그 추측과 매우 가까운 변형 후보도 함께 버리고, 왜 틀렸는지 역추론한 뒤 다음 질문을 해라.
